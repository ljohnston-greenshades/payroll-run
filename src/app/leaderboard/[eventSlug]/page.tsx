...
// "hr-tech-2026" → "Hr Tech 2026". Best-effort title-case for pre-
// events-table data where there's no canonical display name on file.
...
export default async function EventLeaderboardPage({
  ...
  if (!event && !(await eventHasData(slug))) {
    notFound();
  }

  const isCurrent = !!event && !event.archived_at;
  const displayName = event?.name ?? prettifySlug(slug);
